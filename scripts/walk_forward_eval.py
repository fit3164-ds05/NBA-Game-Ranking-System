...
def eval_cls(games, X_cols, ratings_kind):
    per_year = []
    model = XGBClassifier(
        n_estimators=900,
        learning_rate=0.05,
...
def eval_reg(games, X_cols, ratings_kind):
    per_year = []
    model = XGBRegressor(
        n_estimators=1200,
        learning_rate=0.05,
...
def main():
    ...
    games, X_cols = build_features(ratings_kind=rk)
    if task == "cls":
        eval_cls(games, X_cols, rk)
    else:
        eval_reg(games, X_cols, rk)
